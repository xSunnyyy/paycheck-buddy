    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.bg }}>
      ...
      >
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
          <Text style={{ color: COLORS.textStrong, ...TYPE.h1 }}>
            Paycheck Buddy
          </Text>

          <View style={{ flexDirection: "row", gap: 8 }}>
            <TextBtn
              label={screen === "checklist" ? "Checklist" : "Checklist"}
            ...
            <>
              ...
              <Card>
                <Text style={{ color: COLORS.muted, ...TYPE.body }}>
                  {cycle.label} • Payday: <Text style={{ color: COLORS.textStrong }}>{formatDate(cycle.payday)}</Text>
                </Text>
                              ...
                              <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 10 }}>
                                <View style={{ flex: 1 }}>
                                  ...
                                  <Text style={{ color: COLORS.muted, marginTop: 4, fontWeight: "700" }}>
                                    {fmtMoney(it.amount)}{it.notes ? ` • ${it.notes}` : ""}
                                    {isChecked && state?.at ? ` • checked ${new Date(state.at).toLocaleString()}` : ""}
                                  </Text>
                                </View>
...
function SettingsScreen({
  ...
  function addBill() {
    ...
    setLocal((s) => ({
      ...
      bills: [
        ...(s.bills || []),
        { id, name: "New Bill", amount: 0, dueDay: 1 },
      ],
    }));
  }
          ...
          {(local.bills || []).map((b) => (
            ...
            >
              ...
              <Field
                ...
                onChangeText={(s) =>
                  updateBill(b.id, { dueDay: clamp(safeParseNumber(s), 1, 31) })
                }
                keyboardType="numeric"
                placeholder="25"
              />
